...
# default the one on PATH) and cjpeg from libjpeg-turbo. The trimmed
# build cannot encode video, so generation always rides a full build;
# the corpus is generated fresh rather than committed.
set -eu
